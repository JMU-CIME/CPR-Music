...
export default function GradePerform({submissions}) {
  ...
  return <Row>
    <Col>
      { submissions ? 
        <h2>Grading: {submissions?.[0].assignment?.part?.piece?.name} - {submissions?.[0].assignment.activity.activity_type.category} Activity</h2>
        : <h2>No Submissions to Grade</h2>
      }
      {/* TODO: should I show the flat score viewer during grading of perform? */}
      {/* <FlatEditor height={200} /> */}
      { submissions && submissions.map((submission, submissionIdx) => <Row className={[{
        'border': isFormFocused}, 'border-primary', 'border-3']} key={submission.id}>
        <Col>
          <Card>
            <Card.Body>
              <Card.Title>{submission.assignment.enrollment.user.name}</Card.Title>
              <Card.Text>
                { submission.attachments?.[0]?.file && <audio controls src={submission.attachments[0].file} ref={audioRef} /> }
              </Card.Text>
            </Card.Body>
            <ListGroup className="list-group-flush">
              {/* TODO: what time should I show here? */}
              { submission.attachments?.[0]?.submitted && <ListGroupItem><time dateTime={submission.attachments[0].submitted}>{submission.attachments[0].submitted}</time></ListGroupItem> }
              <ListGroupItem>{submission.id}</ListGroupItem>
              { submission?.assignment?.activity?.activity_type?.category === 'Create' && submission?.content && <ListGroupItem><FlatEditor scoreJSON={JSON.parse(submission.content)}/></ListGroupItem> }
              <ListGroupItem>{submission.attachments.length} submission{submission.attachments.length === 1 ? '' : 's'}</ListGroupItem>
            </ListGroup>
          </Card>
        </Col>
